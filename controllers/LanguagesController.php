...
class LanguagesController extends BackendController {
    ...
    public function actionCreate() {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            } else if(empty($model->is_default)){
                $total = Languages::find()->where(array('is_default' => 1))->count();
                if(empty($total))
        ...
        if (Yii::$app->getRequest()->isAjax) {
            ...
            if (!empty($languageList)) {
                foreach ($languageList as $languageId) {
                    ...
                    if ($language) {
                        PageContent::deleteAll(['language' => $language->primaryKey]);
                        QuickLinkLanguage::deleteAll(['language_id' => $language->primaryKey]);
                        QuickLinkGroupLanguage::deleteAll(['language_id' => $language->primaryKey]);
                        $language->delete();
                    }
                }
    ...
    public function actionLanguagesetting() {
        ...
        if (empty($language_array)) {
            Yii::$app->session->setFlash('warning', Yii::t('app', 'Please define language'));
        ...
        if (empty($source_messages)) {
            Yii::$app->session->setFlash('error', Yii::t('app','Please define message in database'));
        }
    ...
    public function actionSavelangmessages() {
        ...
        if (Yii::$app->request->isAjax) {
            ...
            if (!empty($messages) && !empty($messages_id)) {
                ...
                if ($flag) {
                    $data['success'] = 1;
                    $data['notification'] = Yii::t('app','Messages save successfully');
                } else {
                    $data['error'] = 0;
                    $data['notification'] = Yii::t('app','Has some messages cannot save');
                }
            } else {
                $data['error'] = 0;
    ...
    public function actionDisplaylanguage() {
        ...
        $source_messages = SourceMessage::find()->all();
        if (empty($source_messages)) {
            Yii::$app->session->setFlash('error', Yii::t('app','Please define source message'));
